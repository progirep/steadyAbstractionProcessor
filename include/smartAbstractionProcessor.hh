#ifndef __SMART_ABSTRACTION_PROCESSOR_HH__
#define __SMART_ABSTRACTION_PROCESSOR_HH__
...
template<class ConcreteSCOTSAbstraction, class ConcreteWorkspaceProperties, class ConcreteSATSolver, bool restrictSpreading, bool removeDominant> class SmartAbstractionProcessor {
private:
...
public:
    SmartAbstractionProcessor(ConcreteSCOTSAbstraction, ConcreteWorkspaceProperties);
    void convertNormalTransitionIntoSteadyTransitions();
    void computeAbstraction();
};

template<class ConcreteSCOTSAbstraction, class ConcreteWorkspaceProperties, class ConcreteSATSolver, bool restrictSpreading, bool removeDominant> SmartAbstractionProcessor<ConcreteSCOTSAbstraction,ConcreteWorkspaceProperties,ConcreteSATSolver,restrictSpreading, removeDominant>::SmartAbstractionProcessor(ConcreteSCOTSAbstraction _SCOTSAbstraction, ConcreteWorkspaceProperties _workspaceProperties) :
    SCOTSAbstraction(_SCOTSAbstraction), workspaceProperties(_workspaceProperties)
{}


template<class ConcreteSCOTSAbstraction, class ConcreteWorkspaceProperties, class ConcreteSATSolver, bool restrictSpreading, bool removeDominant> void SmartAbstractionProcessor<ConcreteSCOTSAbstraction,ConcreteWorkspaceProperties,ConcreteSATSolver,restrictSpreading, removeDominant>::computeAbstraction() {
    ...
    std::array<double,ConcreteSCOTSAbstraction::ConcreteDynamics::getStateNofDimensions()> maxSpreadingFactor = workspaceProperties.getMaxSpreadingFactors();
    ...
    const std::array<bool,ConcreteSCOTSAbstraction::ConcreteDynamics::getStateNofDimensions()> dimensionIsWrapAround = ConcreteSCOTSAbstraction::ConcreteDynamics::getDimensionIsWrapAround();
    ...
    for (typename std::unordered_map<state_type, std::map<input_type,std::list<state_type>>,state_type_hasher>::const_iterator transitionElement = transitionMap.begin(); transitionElement != transitionMap.end(); transitionElement++) {
        ...
        std::array<size_t,nofDimensionsPerState> maxSpread = {};

        if (restrictSpreading) {
            // Obtain worst-case spreading factors in every dimension
            for (unsigned int i=0;i<nofDimensionsPerState;i++) maxSpread[i] = 0.0;

            for (auto const &startingMicrocell : microcellsOfStartingMacrocell) {
                try {
                    for (auto const &cInputMap : transitionMap.at(startingMicrocell)) {
                        std::array<std::set<int>,nofDimensionsPerState> valuesFound;

                        for (state_type nextState : cInputMap.second) {
                            for (unsigned int i=0;i<nofDimensionsPerState;i++) {
                                int thisValue = nextState[i] / splittingFactor[i];
                                valuesFound[i].insert(thisValue);
                            }

                        }
                        for (unsigned int j=0;j<nofDimensionsPerState;j++) maxSpread[j] = std::max(maxSpread[j],valuesFound[j].size());
                    }
                } catch (const std::out_of_range& oor) {
                    // Ok.
                }
            ...
            // Now enforce that the solutions may not have transitions that have successor nodes that are more that maxSpreadingFactor*maxSpread
            // away.
            unsigned int nofAddedConstraints = 0;
            for (auto it : successorMacrostateToSATVarMapper) {
                for (auto it2 : successorMacrostateToSATVarMapper) {

                    bool foundTooMuchSpread = false;
                    for (unsigned int i=0;i<nofDimensionsPerState;i++) {
                        if (dimensionIsWrapAround[i]) {
                            int spread = rotMin(it.first[i]-it2.first[i],nofGridPointsStateCoarse[i]);
                            foundTooMuchSpread |= spread>maxSpread[i]*maxSpreadingFactor[i];
                        } else {
                            foundTooMuchSpread |= std::abs(it.first[i]-it2.first[i])>maxSpread[i]*maxSpreadingFactor[i];
                        }
                    }

                    if (foundTooMuchSpread) {
                        s.add(-1*it.second);
                        s.add(-1*it2.second);
                        s.add(0);
                        nofAddedConstraints++;
            ...
            std::cerr << "(R.S. added clauses: " << nofAddedConstraints << ")";
        }
        ...
        // Now filter some of them out -- if we switched on filtering, that is.
        if (restrictSpreading) {

            // Idea: Reject a move if all of the next states are covered already by more
            // compact moves. Moves of the same compactness level are OK.

            // First, order everything by value
            std::cerr << "# Actions before filtering: " <<steadyActionsComputed.size() << std::endl;
            std::map<int,std::list<std::pair<std::set<state_type>,std::map<state_type,input_type>>>> moveTargetNodeDistances;

            for (auto it = steadyActionsComputed.begin();it!=steadyActionsComputed.end();it++) {

                // Compute max Distance
                int maxDist = 0;
                std::set<state_type> visitedStates;
                for (auto const &startingMicrocell : microcellsOfStartingMacrocell) {
                    for (state_type nextState : transitionMap.at(startingMicrocell)[(*it)[startingMicrocell]]) {
                        std::array<int,nofDimensionsPerState> valuesFound;
                        for (unsigned int i=0;i<nofDimensionsPerState;i++) {
                            int thisValue = nextState[i] / splittingFactor[i];
                            valuesFound[i] = thisValue;
                        }
                        visitedStates.insert(valuesFound);
                    }
                }

                for (auto stateIt1 : visitedStates) {
                    for (auto stateIt2 : visitedStates) {

                        // Compute dist
                        int dist = 0;
                        for (unsigned int i=0;i<nofDimensionsPerState;i++) {
                            int value1 = stateIt1[i];
                            int value2 = stateIt2[i];

                            if (dimensionIsWrapAround[i]) {
                                dist += rotMin(value1-value2,nofGridPointsStateCoarse[i]);
                            } else {
                                dist += std::abs(value1-value2);
                            }
                        }
                    }
                }

                moveTargetNodeDistances[maxDist].push_back(std::pair<std::set<state_type>,std::map<state_type,input_type>>(visitedStates,*it));
            }

            // Now Add the transitions that we want back in.
            //unsigned int oldSize = cInputToMacrocellsMap.size();
            steadyActionsComputed.clear();
            std::set<state_type> coveredStates;
            //std::set<state_type> coveredStatesLast;

            for (auto &itOuter : moveTargetNodeDistances) {
                for (auto &it : itOuter.second) {

                    // double currentCost = it.first;

                    // Iterate over the states
                    bool anyUnvisitedState = false;
                    for (auto &targetState : it.first) {
                        if (coveredStates.count(targetState)==0) {
                            // Ok, so this one is now covered
                            anyUnvisitedState = true;
                            coveredStates.insert(targetState);
                        }
                    }

                    if (anyUnvisitedState) {
                        steadyActionsComputed.push_back(it.second);
                    }

                    //coveredStatesLast = coveredStates;
                }
            }
            std::cerr << "# Actions after filtering: " <<steadyActionsComputed.size() << std::endl;
        }
