...
/**
 * @brief Implementation of CUDD's VectorCompose function.
 * @param composeVector The assignment of all BDD variables to BDDs to be replaced.
 * @return
 */
BFBdd BFBdd::compose(BFBddComposeVector &composeVector) const {
    composeVector.ensureSufficientSize();
    DdNode *res = Cudd_bddVectorCompose(mgr,node,composeVector.nodes);
    return BFBdd(bfmanager, res);
}
