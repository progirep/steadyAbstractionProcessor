...
private:
    ...
    std::vector<BF> &vars;
public:
    TrivialVariableInfoContainer(BFManager &_mgr, std::vector<std::string> &_varNames, std::vector<BF> &_vars) : /* mgr(_mgr), */ varNames(_varNames), vars(_vars) { (void)_mgr; };
    void getVariableTypes(std::vector<std::string> &types) const { types.push_back("all"); }
    void getVariableNumbersOfType(std::string type, std::vector<unsigned int> &nums) const { (void)type; for (unsigned int i=0;i<varNames.size();i++) nums.push_back(i); }
